...
class SetupWizard:
    ...
    def run(self, rerun: bool = False) -> bool:
        ...
        if self.progress.has_progress():
            ...
            print("  If you've updated the code or want to review all settings,")
            print("  starting over is recommended. Your previous answers are")
            print("  saved in config.yml and will appear as defaults.")
            choice = Menu.prompt_choice(
                "What would you like to do?",
                ["Start over from the beginning", "Resume where you left off"],
            )
            if choice == 0:
                self.progress.clear()
            ...
            try:
                ...
                while not success:
                    ...
                    if choice == 0:
                        ...
                        # Skip
                        print(f"  Skipped phase {phase_num}. You can configure")
                        print(f"  this later from the interactive menu.")
                        break
                    else:
                        # Save and exit
                        self.progress.save()
    ...
    def _phase_welcome(self, rerun: bool = False) -> bool:
        ...
        # Display system information for the user to verify
        ...
        print(f"\n  System Information:")
        print(f"    Operating System: {os_name} {os_release} ({os_arch})")
        ...
        if sys.version_info < (3, 10):
            ...
            print(f"  You have Python {py_version}.")
            print("  Please upgrade Python and try again.")
            return False

        print(f"\n  Requirements: Python 3.10+ (you have {py_version}) — OK")

        if not Menu.prompt_yes_no("Does this look correct?"):
            print("  If your Python version or location is wrong, exit and")
            print("  activate the correct virtual environment before retrying.")
            return False
    ...
    def _phase_output_format(self, rerun: bool = False) -> bool:
        ...
        # Detect which output backends are available based on installed
        # dependencies. The user already chose their backend when they
        # ran `pip install .[excel]` or `pip install .[google]`, so we
        # can often confirm rather than re-ask.
        has_excel = False
        has_google = False
        try:
        ...
        if has_excel and not has_google:
            # Only Excel libraries installed — confirm
            print("\n  Excel dependencies detected (openpyxl is installed).")
            if Menu.prompt_yes_no("Use Microsoft Excel (.xlsx) as your output format?", default=True):
                fmt = "excel"
            else:
                print("  To use Google Sheets instead, run: pip install .[google]")
                fmt = "excel"  # default anyway since Google libs aren't available
        elif has_google and not has_excel:
            # Only Google libraries installed — confirm
            print("\n  Google Sheets dependencies detected.")
            if Menu.prompt_yes_no("Use Google Sheets as your output format?", default=True):
                fmt = "google_sheets"
            else:
                print("  To use Excel instead, run: pip install .[excel]")
                fmt = "google_sheets"  # default since Excel libs aren't available
        elif has_excel and has_google:
            # Both installed — ask which one
            print("\n  Both Excel and Google Sheets dependencies are installed.")
            ...
            # Neither installed — warn and default to Excel
            print("\n  WARNING: Neither Excel (openpyxl) nor Google Sheets")
            print("  dependencies are installed. Run one of:")
            print("    pip install .[excel]")
            print("    pip install .[google]")
            print("  Defaulting to Excel format.")
            fmt = "excel"
    ...
    def _phase_archivesspace(self, rerun: bool = False) -> bool:
        """Phase 3: ArchivesSpace connection with repository selection."""
        ...
        # Test connection and fetch repositories
        print("\n  Testing connection...")
        try:
            ...
            # Temporarily set a placeholder repo ID so the client can connect.
            # The real repo ID is set after the user picks from the list.
            self.config.set("archivesspace", "repository_id", value=1)
            client = ArchivesSpaceClient(self.config, self._logger)

            if client.connect():
                print("  Connection successful!")

                # Fetch and display available repositories
                repos = client.get_repositories()
                if not repos:
                    print("  ⚠ Connected, but no repositories found.")
                    print("  Check that your account has repository access.")
                    if Menu.prompt_yes_no("Retry this step?"):
                        return self._phase_archivesspace(rerun)
                    return False

                # Filter out the global "agent" repository (ID 1) which
                # is an internal ArchivesSpace repository not meant for
                # user access.
                user_repos = [
                    r for r in repos
                    if r.get("repo_code") != "archivesspace_agent"
                ]
                if not user_repos:
                    user_repos = repos  # fallback if filtering removed everything

                print(f"\n  Found {len(user_repos)} repository(ies):\n")
                repo_options = []
                for repo in user_repos:
                    name = repo.get("name", "Unnamed")
                    code = repo.get("repo_code", "")
                    uri = repo.get("uri", "")
                    # Extract ID from URI like "/repositories/2"
                    try:
                        repo_id = int(uri.rstrip("/").split("/")[-1])
                    except (ValueError, IndexError):
                        repo_id = 0
                    repo_options.append((repo_id, name, code))
                    print(f"    ID {repo_id}: {name} ({code})")

                if len(repo_options) == 1:
                    # Only one repository — confirm it
                    repo_id, name, code = repo_options[0]
                    print(f"\n  Only one repository available: {name}")
                    if not Menu.prompt_yes_no("Use this repository?"):
                        print("  You can add more repositories in ArchivesSpace")
                        print("  and re-run this step later.")
                        return False
                else:
                    # Multiple repositories — let user pick
                    choice = Menu.prompt_choice(
                        "Select a repository:",
                        [f"{name} ({code})" for _, name, code in repo_options],
                    )
                    repo_id, name, code = repo_options[choice]

                self.config.set("archivesspace", "repository_id", value=repo_id)
                print(f"\n  Selected repository: {name} (ID {repo_id})")
    ...
    def _phase_google_api(self, rerun: bool = False) -> bool:
        ...
        if choice == 0:
            print()
            print("  To create a service account JSON key file:")
            print()
            ...
            print("         - Search for 'Google Drive API' and click 'Enable'")
            print("    4. Create a service account:")
            print("         - Go to 'APIs & Services' > 'Credentials'")
            print("         - Click 'Create Credentials' > 'Service account'")
            print("         - Give it a name (e.g., 'accession-sync') and click 'Done'")
            ...
            print("         - A .json file will download to your computer")
            print("    6. Move the downloaded file to a safe location.")
            print()
            print("  IMPORTANT: Do not store the key file in a folder that syncs")
            print("  to the cloud (e.g., OneDrive, Dropbox, iCloud). The file")
            print("  contains a private key that grants access to your Google")
            print("  account. Recommended locations:")
            print()
            print("    Windows:  C:\\Users\\YourName\\.credentials\\accession-sync\\")
            print("    macOS:    ~/.credentials/accession-sync/")
            print("    Linux:    ~/.credentials/accession-sync/")
            print()
            print("  You may need to create these folders first. On Windows:")
            print("    mkdir C:\\Users\\YourName\\.credentials\\accession-sync")
            print()
            print("  Then move the downloaded .json file into that folder.")
            print("  Enter the full path to the file below.")
            print()
            ...
            while not Path(key_path).exists():
                ...
                if choice_retry == 1:
                    break  # proceed with the invalid path
                key_path = Menu.prompt_text(
                    "Path to service account JSON key file:",
                    default=key_path,
                )
            ...
            print("         - Choose 'Internal' (if using a Google Workspace account)")
            print("           or 'External' (for personal Gmail accounts)")
            print("         - Fill in the required fields and save")
            print("    5. Create OAuth credentials:")
            print("         - Go to 'APIs & Services' > 'Credentials'")
            print("         - Click 'Create Credentials' > 'OAuth client ID'")
            print("         - Application type: 'Desktop app'")
            print("         - Note the Client ID and Client Secret shown")
            print()
        ...
        # Google Drive folder
        print()
        print("  The spreadsheet will be created in a Google Drive folder.")
        print("  To find a folder's ID:")
        print("    1. Open the folder in Google Drive in your browser")
        print("    2. Look at the URL — it will look like:")
        print("       https://drive.google.com/drive/folders/1aBcDeFgHiJkLmNoPqRsTuVwXyZ")
        print("    3. The folder ID is the part after 'folders/':")
        print("       1aBcDeFgHiJkLmNoPqRsTuVwXyZ")
        print()
        print("  If using a service account, share the folder with the")
        print("  service account's email address (found in the JSON key file")
    ...
    def _phase_extent_types(self, rerun: bool = False) -> bool:
        ...
        print()
        print("  ArchivesSpace records extents in whatever units your institution")
        print("  uses — linear feet, boxes, items, gigabytes, megabytes, etc. But")
        print("  the spreadsheet needs a single total for each accession in a common")
        print("  unit: linear feet for physical extents and gigabytes for digital.")
        print()
        print("  For each extent type, you'll specify:")
        print("    1. Whether it's physical or digital")
        print("    2. A conversion factor — how many of the base unit one of this")
        print("       type equals")
        print()
        print("  For example:")
        print("    - 'linear_feet' is physical with factor 1.0 (already the base unit)")
        print("    - 'boxes' might be physical with factor 1.5 (if 1 box ≈ 1.5 LF)")
        print("    - 'items' might be physical with factor 0.01 (rough estimate)")
        print("    - 'gigabytes' is digital with factor 1.0 (already the base unit)")
        print("    - 'megabytes' is digital with factor 0.001 (1 MB = 0.001 GB)")
        print("    - 'terabytes' is digital with factor 1000 (1 TB = 1,000 GB)")
        print()
        print("  The program will multiply the quantity by the factor to convert:")
        print("  e.g., an accession with '3 boxes' at factor 1.5 = 4.5 linear feet.")
        print()
        print("  Note on digital units: The default factors use the metric standard")
        print("  (1 GB = 1,000 MB). Computers and Windows file properties use the")
        print("  binary standard (1 GiB = 1,024 MiB), which is about 7% smaller at")
        print("  the gigabyte level. For backlog prioritization the difference is")
        print("  negligible, but if your measurements come from Windows and you want")
        print("  precise conversions, use 0.000977 instead of 0.001 for megabytes")
        print("  and 1024 instead of 1000 for terabytes.")
