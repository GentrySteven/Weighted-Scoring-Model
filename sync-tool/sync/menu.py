...
MENU_STRUCTURE = {
    ...
    "Administration": [
        ...
        ("26", "Verify configuration", "Check settings and verify repository connection", "verify_config", False),
...
HELP_TEXT = {
    ...
    "verify_config": (
        ...
        "  and credential files. Also connects to ArchivesSpace to verify\n"
        "  which repository you're pointed at, and lets you switch to a\n"
...
class Menu:
    ...
    def _run_tour(self) -> None:
        ...
        print("  explain what each feature does. You can skip any section.")
        print()
        print("  Each feature is shown with its menu location so you can")
        print("  find it later during day-to-day use.")
        ...
        phases = [
            ("Phase 1 — Verify Your Setup", [
                "verify_config", "dry_run", "last_sync",
            ]),
            ("Phase 2 — Running Syncs", [
                "sync", "review_preview", "reconfigure",
            ]),
            ("Phase 3 — Customizing Data Management", [
                "manage_extents", "manage_triggers", "manage_dropdowns",
                "manage_vocabs",
            ]),
            ("Phase 4 — Building Vocabularies", [
                "scan_formats", "scan_subjects", "scan_issues",
                "manage_subjects",
            ]),
            ("Phase 5 — Scoring & Processing Queues", [
                "view_scoring", "edit_scoring", "view_queues", "edit_queues",
            ]),
            ("Phase 6 — Scheduling & Notifications", [
                "schedule_sync", "schedule_dry_run", "check_updates",
            ]),
            ("Phase 7 — Monitoring & Maintenance", [
                "view_logs", "view_log_storage", "consolidate_logs",
                "view_backups", "view_cache", "clear_cache", "manage_previews",
            ]),
        ]

        # Actions that should never offer "Try this now?" during the tour.
        # reconfigure: the user just finished the wizard — rerunning it
        # immediately is pointless and confusing.
        skip_try_now = {"reconfigure"}
            ...
            for action_key in action_keys:
                ...
                for _category, options in MENU_STRUCTURE.items():
                    for key, label, desc, ak, info_only in options:
                        if ak == action_key:
                            print(f"\n  {label}")
                            print(f"  (Menu: {_category} > Option {key})")
                            detailed = HELP_TEXT.get(ak, desc)
                            print(f"  {detailed}")

                            if ak == "reconfigure":
                                print("  (Use this later if you need to change")
                                print("  settings established during initial setup.)")

                            if ak in self._actions and not info_only and ak not in skip_try_now:
    ...
    def prompt_text(question: str, default: str = "") -> str:
        ...
        # Strip any trailing colon/whitespace from the question so the
        # method's own ": " suffix doesn't produce a double colon.
