...
class ArchivesSpaceClient:
    ...
    def get_repositories(self) -> list[dict]:
        """
        Retrieve all repositories from the ArchivesSpace instance.

        Returns a list of repository dicts, each containing at minimum
        'uri', 'repo_code', and 'name'. The repository ID can be
        extracted from the URI (e.g., '/repositories/2' → 2).

        This endpoint does not require a repository to be selected,
        so it can be called before the user has chosen a repository.
        """
