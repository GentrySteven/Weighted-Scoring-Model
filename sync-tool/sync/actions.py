...
def _action_verify_config(config: ConfigManager) -> None:
    """Verify configuration status and offer repository switching."""
    ...
    # Attempt to fetch the current repository name live from ArchivesSpace
    # and offer to switch if the user is connected to the wrong one.
    ...
    try:
        ...
        if not client.connect():
            ...
            print("  Check your API URL and credentials.")
            return

        repos = client.get_repositories()
        current_id = config.get_repository_id()
        current_repo = None
        user_repos = [
            r for r in repos
        ...
        if current_repo:
            name = current_repo.get("name", "Unknown")
            code = current_repo.get("repo_code", "")
            print(f"  Connected to: {name} ({code}, ID {current_id})")
        else:
            print(f"  ⚠ Current repository ID ({current_id}) was not found")
            print(f"  on this ArchivesSpace instance.")

        if len(user_repos) > 1:
            if not Menu.prompt_yes_no("Is this the correct repository?"):
                # Offer to switch
                print(f"\n  Available repositories:\n")
                repo_options = []
                for r in user_repos:
                    name = r.get("name", "Unnamed")
                    code = r.get("repo_code", "")
                    uri = r.get("uri", "")
                    try:
                        rid = int(uri.rstrip("/").split("/")[-1])
                    except (ValueError, IndexError):
                        rid = 0
                    repo_options.append((rid, name, code))

                choice = Menu.prompt_choice(
                    "Select a repository:",
                    [f"{name} ({code})" for _, name, code in repo_options],
                )
                new_id, new_name, new_code = repo_options[choice]
                config.set("archivesspace", "repository_id", value=new_id)
                config.save_config()
                print(f"\n  Switched to: {new_name} ({new_code}, ID {new_id})")
                print("  This change takes effect on the next sync.")

    except ImportError:
        print("  ArchivesSnake not installed — cannot verify repository.")
    except Exception as e:
        print(f"  ⚠ Could not verify repository: {e}")
