...
def main():
    ...
    # First run detection — check both file existence and whether
    # essential settings have been configured. A template config file
    # with blank values should trigger the wizard just like a missing file.
    needs_wizard = False
    if not config.config_exists():
        ...
        try:
            config.load()
            # Check if critical settings are actually populated
            base_url = config.get("archivesspace", "base_url", default="")
            output_format = config.get("output", "format", default="")
            if not base_url or not output_format:
                needs_wizard = True
        except Exception:
            needs_wizard = True
    ...
    if needs_wizard:
        ...
        if config.config_exists():
            print("  Configuration file found but essential settings are missing.")
            print("  The setup wizard will walk you through completing your setup.\n")
        else:
        ...
        choices = ["Guided setup wizard"]
        if not config.config_exists():
            choices.append("Create template files")
        choices.append("Skip to interactive menu")

        choice = Menu.prompt_choice("Choose:", choices)

        if choices[choice] == "Guided setup wizard":
            ...
            if wizard.run(rerun=False):
                ...
                return
        elif choices[choice] == "Create template files":
            _create_templates(config)
            ...
            return
        # "Skip to interactive menu" falls through to the menu below
