...
This tool lives in the `sync-tool` directory of the [Weighted-Scoring-Model](https://github.com/GentrySteven/Weighted-Scoring-Model) repository alongside non-automated versions of the scoring model:

```
Weighted-Scoring-Model/
├── README.md                      # Repository overview
├── sync-tool/                     # ← This tool (automated, Python-based)
│   ├── sync/
│   ├── pyproject.toml
│   └── ...
└── (other directories)            # Non-automated scoring model versions
```
...
│   ├── runner.py              # Sync pipeline orchestration
│   ├── actions.py             # Interactive menu action handlers
│   ├── supporting_sheets.py   # Supporting sheet and visualization writers
...
│   ├── scoring_criteria.py    # Scoring criteria sheet builder
│   ├── processing_queue.py    # Processing queue and Backlog At a Glance builders
│   ├── visualizations.py      # Visualization data tables and chart specifications
...
The tool lives inside the `sync-tool` subdirectory of the repository:
...
You should now be in the directory containing `pyproject.toml`. You can verify with:

**Windows:** `dir pyproject.toml`

**macOS / Linux:** `ls pyproject.toml`

If you see "File Not Found" or "No such file", you're in the wrong directory.

### Step 3: Create a virtual environment

This keeps the tool's dependencies isolated from your system Python:
...
This is a **separate command** from Step 3. After running it, you should see `(venv)` at the beginning of your command prompt.
...
Make sure you are in the directory that contains `pyproject.toml` (the `sync-tool` directory from Step 2) and that your virtual environment is active (you should see `(venv)` in your prompt). Then run **one** of the following:
